...
PAGES = [
    ...
    },
    {
        ...
        "caveat": "Документация ЦРПТ открыта: "
                  "<a href=\"https://docs.crpt.ru/gismt/True_API/\">docs.crpt.ru/gismt/True_API/</a>. "
                  "А вот машиночитаемой спеки нет, <code>/api/v3/true-api/swagger.json</code> "
                  "отдаёт 401, поэтому каталог не сгенерирован из спеки, а собран по "
                  "открытым SDK. У каждой записи стоит <code>verified: false</code>: пути "
                  "надёжные, глаголы и параметры сверяются с документацией и на живом "
                  "контуре. Сервер показывает этот статус в describe_method, чтобы агент "
        ...
        "errors": [
            ...
            ("Метод отвечает не тем, что описано в карточке",
             "Возможно. У всех 33 записей стоит пометка «не проверено»: каталог собран по "
             "открытым SDK, а не сгенерирован из машиночитаемой спеки, которой у ЦРПТ нет. "
             "Сверяйте с документацией, она открыта: docs.crpt.ru/gismt/True_API/. Сервер "
             "показывает статус в describe_method, чтобы агент не выдавал догадку за факт."),
        ],
        "faq": [
            ...
            ("Почему методы помечены непроверенными?",
             "Машиночитаемой спеки у ЦРПТ нет, /api/v3/true-api/swagger.json отдаёт 401, "
             "поэтому каталог собран по открытым SDK, а не сгенерирован автоматически. "
             "Документация при этом открыта на docs.crpt.ru/gismt/True_API/, и честнее "
             "пометить каталог разведочным, чем выдать его за спецификацию."),
...
def index_page() -> str:
    ...
    faq = [
        ...
        ("Почему у Честного знака методы помечены непроверенными?",
         "Машиночитаемой спеки у ЦРПТ нет, поэтому пути взяты из открытых SDK, а не "
         "сгенерированы автоматически. Документация открыта на docs.crpt.ru, сверить "
         "есть с чем, и сервер честно показывает статус вместо догадки."),
